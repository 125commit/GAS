...
//customed func to fill the  struct
void UAuraAttributeSet::SetEffectPropertied(const FGameplayEffectModCallbackData& Data, FEffectProperties& Props) const
{
...
//Source = source of the effect, Target = target of the effect (owner of this AS,like player)
//After the second when GE was applied,this func will be called
void UAuraAttributeSet::PostGameplayEffectExecute(const FGameplayEffectModCallbackData& Data)
{
	Super::PostGameplayEffectExecute(Data);

	FEffectProperties Props;
	SetEffectPropertied(Data, Props);
	...
	if (Data.EvaluatedData.Attribute == GetIncomingDamageAttribute())
	{
		...
		if (LocalIncomingDamage > 0.f)
		{
			...
			const bool bBlock =  UAuraAbilitySystemLibrary::IsBlockHit(Props.ContextHandle);
			const bool bCriticalHit = UAuraAbilitySystemLibrary::IsCriticalHit(Props.ContextHandle);
	...
	if (Data.EvaluatedData.Attribute == GetIncomingXPAttribute())
	{
		...
		if (Props.SourceCharacter->Implements<UPlayerInterface>())
		{
			...
			{
				const int32 AttribuetPointsReward = IPlayerInterface::Execute_GetAtttributePointsReward(Props.SourceCharacter, CurrentLevel);
				const int32 SpellPointsReward = IPlayerInterface::Execute_GetSpellPointsReward(Props.SourceCharacter, CurrentLevel);

				IPlayerInterface::Execute_AddToPlayerLevel(Props.SourceCharacter, NewLevel - CurrentLevel);
				IPlayerInterface::Execute_AddToAttributePoints(Props.SourceCharacter, AttribuetPointsReward);
...
void UAuraAttributeSet::ShowFloatingText(const FEffectProperties& Props, float Damage, bool bBlockHit, bool bCriticalHit)
{
	if (Props.SourceCharacter != Props.TargetCharacter)
	{
		...
		{
			PC->ShowDamageText(Damage, Props.TargetCharacter, bBlockHit, bCriticalHit);
			return;
		}
...
void UAuraAttributeSet::SendXPEvent(const FEffectProperties& Props)
{
	if (Props.TargetCharacter->Implements<UCombatInterface>())
	{
		...
		const FAuraGameplayTags& GameplayTags = FAuraGameplayTags::Get();
			//Set Payload which will be sent
